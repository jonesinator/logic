...
/// A gate made from transistors that performs the NAND function.
#[derive(Device)]
pub struct NandGate {
    #[child]
...
impl NandGate {
    /// Construct a new NAND gate with the given number of inputs (which must be greater than 2).
    pub fn new(num_inputs: usize) -> Self {
        // Check the arguments.
        if num_inputs < 2 {
            panic!("NAND gate must have two or more inputs.");
        }
        ...
        pmos.iter()
            .for_each(|p| Pin::connect(strong_true.get_output(), p.get_source()));

        // The remaining nmos are connected in a chain.
        zip(nmos.iter(), nmos[1..].iter())
            .for_each(|(n0, n1)| Pin::connect(n0.get_drain(), n1.get_source()));

        // All of the pmos drains are connected together (by connecting them all to drain 0).
        pmos[1..]
            .iter()
            .for_each(|t| Pin::connect(t.get_drain(), pmos[0].get_drain()));

        // The pmos drains are connected to the final nmos drain.
        Pin::connect(pmos[0].get_drain(), nmos[num_inputs - 1].get_drain());
...
mod tests {
    ...
    fn test_nand_gate_3_input() {
        ...
        let mut nand_gate = NandGate::new(3);
        let mut test_pin_a = TestPin::new(DriveValue::HighImpedance);
        let mut test_pin_b = TestPin::new(DriveValue::HighImpedance);
        let mut test_pin_c = TestPin::new(DriveValue::HighImpedance);
        Pin::connect(test_pin_a.get_output(), &nand_gate.get_input()[0]);
        Pin::connect(test_pin_b.get_output(), &nand_gate.get_input()[1]);
        Pin::connect(test_pin_c.get_output(), &nand_gate.get_input()[2]);
